...
class InsertDiscountRequest {
  ...
  static validate(data) {
    ...
    const schema = Joi.object({
      ...
      code: Joi.string().required(), // ➡ Mỗi dòng là một trường dữ liệu cần kiểm tra.
      percent_value: Joi.number().min(0).max(100).required(),
      max_discount: Joi.number().min(0).required(),
      min_total: Joi.number().min(0).required(),
...
module.exports = InsertDiscountRequest
